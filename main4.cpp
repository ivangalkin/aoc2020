...
//"cid",
         ...
         assert (it != std::end(names));
         return std::distance(std::begin(names), it);
     }
     void find(const std::string& name, const std::string& value) {

         if (name == "byr" ) {
             auto year = stoull(value, nullptr, 10);
         ...
         assert(name != "hgt" || (value.length() == 3) || (value.length() == 2));
         if (name == "hgt" && value.length() == 3) {
             auto height = stoull(value, nullptr, 10);
             if (height < 150 || height > 193) {
                 return;
             }
         }
         if (name == "hgt" && value.length() == 2) {
             auto height = stoull(value, nullptr, 10);
         ...
         assert(name != "hcl" || (value.length() == 7));
         assert(name != "pid" || (value.length() == 9));
         fields[idx(name)] = true;
     }
 };
//byr (Birth Year)
//iyr (Issue Year)
...
int _4() {
    //std::regex r {"(byr|iyr|eyr|hgt|hcl|ecl|pid|cid):(\\S+)"};
    //std::regex r {"(byr|iyr|eyr|hgt|hcl|ecl|pid):(\\S+)"};
    std::regex r {
        "((byr):(\\d\\d\\d\\d)(\\s|$))|"
        "((iyr):(20\\d\\d)(\\s|$))|"
        "((eyr):(20\\d\\d)(\\s|$))|"
    ...
    for (auto line : input) {
        ...
        {
            size_t nr_of_alternation = -1;
            for (size_t i = 1; i < sm.size(); ++i)
                if (sm[i].str() == sm[0].str()) {
                    nr_of_alternation = i;
                    break;
                }

            e.find(sm[nr_of_alternation+1], sm[nr_of_alternation+2]);
            line = sm.suffix();
        }
    }
