...
struct Bag {
    std::string color;
    std::map<std::string, size_t> can_contain;
    std::set<std::string> can_be_contained;
};

size_t currrent_id = 0;
...
std::vector<BagInstance> instantiation;


Bag& getOrCreateBag(const std::string& color ) {
    if (auto it = bags.find(color); it != bags.end()) {
        return it->second;
    }
    Bag b { color, {}, {}};
    auto [it2, flag] = bags.emplace(color, std::move(b));
    return it2->second;
}
...
void findParents(const std::string& color) {
    auto& bag = bags[color];
    result1.insert(bag.can_be_contained.begin(), bag.can_be_contained.end());
    for (const auto& parent : bag.can_be_contained) {
        findParents(parent);
    }
}

void findChildrenStep0(Bag& parent, size_t nr_of_parens) {
    for (auto & [child_name, child_nr] : parent.can_contain) {
        BagInstance instance { child_name, nr_of_parens * child_nr};
        instantiation.push_back(std::move(instance));
        findChildrenStep0(bags[child_name], nr_of_parens * child_nr);
    }
}

void findChildrenStep1() {
...
int _7() {
    std::regex r_start {"(.*) bags contain"};
    std::regex r_contains { "(\\d+) ([^,]*) bag([s]{0,1})(,|.)"};


    for ( auto line : input) {
        std::smatch sm;

        auto found_root_bag = std::regex_search(line, sm, r_start);
        assert(found_root_bag);
        Bag& root_bag = getOrCreateBag(sm[1].str());
        std::cout << sm[1] << ": ";
        line = sm.suffix();

        while (std::regex_search(line, sm, r_contains)) {
            assert(stoull(sm[1].str(), nullptr, 10) != 0);
            std::cout << sm[1].str() << " '" << sm[2] << "', ";

            size_t old_nr = root_bag.can_contain[sm[2].str()];
            assert(old_nr == 0);
            old_nr += stoull(sm[1].str(), nullptr, 10);
            root_bag.can_contain[sm[2].str()] = old_nr;

            getOrCreateBag(sm[2].str()).can_be_contained.insert(root_bag.color);
    ...
    findChildrenStep0(bags["shiny gold"],1);
    findChildrenStep1();
